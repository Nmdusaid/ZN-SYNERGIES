...
export function FleetShowcase() {
  return (
    <section id="fleet" className="py-20 md:py-48 px-6 relative bg-black overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16 md:mb-40 reveal-on-scroll">
          <h2 className="text-white/30 font-bold tracking-[0.4em] md:tracking-[0.5em] uppercase text-[9px] md:text-xs mb-4 md:mb-6">Hardware & Infrastructure</h2>
          <h3 className="text-[clamp(2rem,7vw,5.5rem)] font-black text-white mb-6 md:mb-8 tracking-tighter leading-[0.95] md:leading-[0.9] uppercase">THE MODERN FLEET</h3>
          <div className="w-16 md:w-20 h-px bg-white/20 mx-auto" />
        </div>
          ...
          {fleet.map((item, idx) => {
            ...
            return (
              <div key={idx} className={cn(
                "flex flex-col items-center gap-10 md:gap-32 reveal-on-scroll",
                ...
                {/* Text Component */}
                <div className="flex-1 space-y-4 md:space-y-12 w-full text-left">
                  <div className="space-y-2 md:space-y-8">
                    <span className="text-white/10 font-black text-[clamp(3.5rem,12vw,10rem)] block leading-none tracking-tighter">
                      0{idx + 1}
                    </span>
                    <h4 className="text-[clamp(1.5rem,5vw,3.5rem)] font-black text-white tracking-tighter uppercase leading-[1.1] md:leading-tight">
                      {item.title}
                    </h4>
                    <p className="text-muted-foreground text-xs md:text-xl font-light leading-relaxed max-w-lg">
                      {item.desc}
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 md:gap-4">
                    {item.specs.map((spec, sidx) => (
                      <div key={sidx} className="px-3 py-1.5 md:px-5 md:py-3 border border-white/10 text-[7px] md:text-[11px] font-black text-white uppercase tracking-[0.15em] md:tracking-[0.2em] bg-white/[0.02]">
                        {spec}
                      </div>
                    ))}
                  </div>
